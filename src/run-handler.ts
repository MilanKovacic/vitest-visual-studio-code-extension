...
export async function runHandler(
  ...
  if (workspaces.length === 0) {
    ...
    vscode.window.showErrorMessage(
      "Cannot run tests: No workspace folder found",
    );
    return;
  }

  if (
    watchers.some((watcher) => TestWatcher.isWatching(watcher.id)) &&
    watchers.length > 0
  ) {
  ...
  await Promise.allSettled(
    workspaces.map(async (folder) => {
      ...
      const testForThisWorkspace = gatherTestItemsFromWorkspace(
        items,
        folder.uri.fsPath,
      );
      if (testForThisWorkspace.length === 0) return;
      ...
      try {
        ...
        if (e instanceof Error) {
          ...
          testForThisWorkspace.forEach((test) =>
            run.errored(test, new vscode.TestMessage(err.toString())),
          );
        }
      }
    }),
...
export async function updateSnapshot(
  ...
  const workspace = determineWorkspaceForTestItems(
    [test],
    vscode.workspace.workspaceFolders,
  );
  const runner = new TestRunner(
    workspace.uri.fsPath,
    getVitestCommand(workspace.uri.fsPath),
  );
...
export async function debugHandler(
  ...
  for (const folder of workspaces) {
    const items = request.include ?? ctrl.items;
    const testsInThisWorkspace = gatherTestItemsFromWorkspace(
      items,
      folder.uri.fsPath,
    );
    if (testsInThisWorkspace.length === 0) continue;
    try {
      const runner = new TestRunner(
        folder.uri.fsPath,
        getVitestCommand(folder.uri.fsPath),
      ...
      if (e instanceof Error) {
        ...
        testsInThisWorkspace.forEach((test) =>
          run.errored(test, new vscode.TestMessage(err.toString())),
        );
      }
    }
...
function gatherTestItems(
  collection: readonly vscode.TestItem[] | vscode.TestItemCollection,
): vscode.TestItem[] {
...
export function gatherTestItemsFromWorkspace(
  collection: readonly vscode.TestItem[] | vscode.TestItemCollection,
  workspace: string,
) {
  return gatherTestItems(collection).filter(
    (item: vscode.TestItem) =>
      item.uri && isPathASubdirectory(workspace, item.uri.fsPath),
  );
}

function determineWorkspaceForTestItems(
  collection: readonly vscode.TestItem[] | vscode.TestItemCollection,
  ...
  if (testItems.length < 1) return workspaces[0];

  const workspace = workspaces.find(
    (workspace) =>
      testItems[0].uri &&
      isPathASubdirectory(workspace.uri.fsPath, testItems[0].uri.fsPath),
  );

  if (!workspace)
    throw new Error(
      "Multiple workspace roots are found; cannot deduce workspace to which these tests belong to",
    );
...
async function runTest(
  ...
  const workspaceFolder = determineWorkspaceForTestItems(
    items,
    vscode.workspace.workspaceFolders!,
  );
  ...
  for (const item of items) {
    ...
    if (!testingData) {
      log.workspaceError(
        ...
        `Item not found ${item.uri?.fsPath}`,
      );
      run.errored(item, new vscode.TestMessage("Item not found"));
      continue;
    }

    if (testingData instanceof TestFile) await testingData.load(ctrl);
    ...
    if (testingData instanceof TestFile) {
      ...
      if (!file) {
        log.workspaceError(
          workspaceFolder.name,
          `File item not found for item ${item.uri?.fsPath}`,
        );
        run.errored(item, new vscode.TestMessage("Item not found"));
        continue;
      }
    ...
    for (const test of getAllTestCases(item)) testCaseSet.add(test);
  }
  ...
  const startDebugProcess = async ({
    args,
    cfg,
    log,
    onProcessEnd: onFinished,
  ...
  const { output, testResultFiles } = await runner!.scheduleRun(
    ...
    items.length === 1 ? WEAKMAP_TEST_DATA.get(items[0])!.getFullPattern() : "",
    {
      info: (msg: string) => {
        if (items.length === 1) run.appendOutput(msg, undefined, items[0]);
        else run.appendOutput(msg);
      },
      error: log.error,
    },
  ...
  if (mode !== "debug" && !cancellation?.isCancellationRequested) {
    for (const item of testCaseSet) {
      ...
      for (const finishedItem of finishedTests) {
        if (finishedItem.id === item.id) testFinished = true;
      }
      if (!testFinished) {
        run.errored(
          item,
