...
import { isDefinitelyVitestEnv, mayBeVitestEnv } from "./pure/is-vitest-env";
import {
  getVitestCommand,
  getVitestVersion,
  isNodeAvailable,
...
export function getRootConfig() {
  ...
  return {
    ...
    disabledWorkspaceFolders: rootConfig.get<string[]>(
...
export async function detectVitestEnvironmentFolders() {
  ...
  for (const folder of vscode.workspace.workspaceFolders) {
    if (
      ((await mayBeVitestEnv(folder)) || getConfig(folder).enable) &&
      !getRootConfig().disabledWorkspaceFolders.includes(folder.name)
    )
...
export async function getVitestWorkspaceConfigs(): Promise<
  VitestWorkspaceConfig[]
> {
  return await Promise.all(
    vitestEnvironmentFolders.map(async (workspace) => {
      const cmd = getVitestCommand(workspace.uri.fsPath);
      const isUsingVitestForSure =
        getConfig(workspace).enable ||
        (await isDefinitelyVitestEnv(workspace)) ||
        !!cmd;

      const version =
        cmd == null
          ? undefined
          : await getVitestVersion(
              cmd,
              getConfig(workspace).env || undefined,
            ).catch(async (e) => {
              log.info(e.toString());
              log.info(`process.env.PATH = ${process.env.PATH}`);
              log.info(
                `vitest.nodeEnv = ${JSON.stringify(getConfig(workspace).env)}`,
              );
              let errorMsg = e.toString();
              if (!isNodeAvailable(getConfig(workspace).env || undefined)) {
                log.info("Cannot spawn node process");
                errorMsg +=
                  'Cannot spawn node process. Please try setting vitest.nodeEnv as {"PATH": "/path/to/node"} in your settings.';
              }
...
function isCompatibleVitestConfig(
  config: Pick<VitestWorkspaceConfig, "version" | "workspace">,
) {
  return !!(
    (config.version && semver.gte(config.version, "0.12.0")) ||
    getConfig(config.workspace).commandLine
  );
}
...
export function getValidWorkspaces(
  ...
  return (
    workspaces.filter(
      (workspace) => !disabledWorkspaceFolders.includes(workspace.name),
    ) || []
  );
}
