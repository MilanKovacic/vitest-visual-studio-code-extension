...
export class TestWatcher extends Disposable {
  ...
  static create(
    ...
    TestWatcher.cache[id] = new TestWatcher(
      id,
      ctrl,
      discover,
      vitest,
  ...
  public async watch() {
    ...
    try {
      if (this.isWatching.value) return;
  ...
  updateStatus() {
    ...
    if (getRootConfig().showFailMessages && failed > 0)
      vscode.window.showErrorMessage(
        `Vitest: You have ${failed} failing Unit Test(s).`,
      );
  }
  ...
  private runFiles(files: File[]): Promise<void> | undefined {
    ...
    if (!this.run)
      this.run = this.ctrl.createTestRun(
...
function parseLocationFromStacks(
  ...
  for (const stack of stacks) {
    const { sourceFilepath, line, column } =
      getSourceFilepathAndLocationFromStack(stack);
...
function testMessageForTestError(
  ...
  if (error?.actual != null && error?.expected != null)
    testMessage = TestMessage.diff(
      error?.message ?? "",
      error.expected,
      error.actual,
    );
  else testMessage = new TestMessage(error?.message ?? "");
...
export function syncTestStatusToVsCode(
    ...
    for (const task of vitest) {
      ...
      if (task.type === "test") {
        if (task.result == null) {
          ...
          if (finishedTest) {
            if (finishedTest.has(data.item)) continue;
          }
          ...
          switch (task.result?.state) {
            ...
            case "fail":
              run.failed(
                data.item,
                task.result.errors?.map((i) =>
                  testMessageForTestError(data.item, i),
                ) ?? [],
  ...
  function matchTask(
    ...
    for (const candidate of candidates) {
      ...
      if (task.type === "test" && !(candidate instanceof TestCase)) continue;
