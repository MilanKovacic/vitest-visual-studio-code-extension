...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (vitestEnvironmentFolders.length === 0) {
    log.info(
      "The extension is not activated because no Vitest environment was detected.",
    );
    return;
  }
  ...
  if (!config) {
    vscode.window.showWarningMessage(
      "Cannot run tests: no Vitest config found.",
    );
    return;
  }
...
function workspacesCompatibilityCheck(
  ...
  workspaceConfigs.forEach((vitest) => {
    log.info(
      `Vitest Workspace [${vitest.workspace.name}]: Vitest version = ${vitest.version}`,
    );
  });

  // prompt error message if we can get the version from vitest, but it's not compatible with the extension
  workspaceConfigs
    .filter((x) => !x.isCompatible && x.isUsingVitestForSure)
    .forEach((config) => {
      vscode.window.showWarningMessage(
        "Because Vitest version < 0.12.0" +
          `, run/debug/watch tests are disabled in workspace "${config.workspace.name}" \n`,
      );
    });

  if (workspaceConfigs.every((x) => !x.isCompatible)) return false;
...
function registerDiscovery(
  ...
  ctrl.resolveHandler = async (item) => {
    if (!item) {
      ...
      if (data instanceof TestFile) await data.updateFromDisk(ctrl);
    }
  };
...
function registerWatchHandlers(
  ...
  const testWatchers =
    vitestConfigs.map((vitestConfig, index) =>
      TestWatcher.create(
        ctrl,
        fileDiscoverer,
        vitestConfig,
        vitestConfig.workspace,
        index,
      ),
    ) ?? [];
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(Command.ToggleWatching, () => {
      const anyWatching = testWatchers.some(
        (watcher) => watcher.isWatching.value,
      );
      if (anyWatching) stopWatching();
      else startWatching();
    }),
  );
...
function registerRunDebugWatchHandler(
  ...
  const workspaces = workspaceConfigs
    .filter((x) => x.isCompatible && !x.isDisabled)
    .map((x) => x.workspace);
  ctrl.createRunProfile(
    "Run Tests",
    vscode.TestRunProfileKind.Run,
