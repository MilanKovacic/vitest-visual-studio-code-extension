...
export class TestFileDiscoverer extends vscode.Disposable {
  ...
  constructor(config: ResolvedConfig) {
    super(() => {
      for (const watch of this.lastWatches) watch.dispose();
  ...
  async watchAllTestFilesInWorkspace(
    ...
    if (!vitestEnvironmentFolders) return []; // handle the case of no opened folders
    ...
    await Promise.all(
      vitestEnvironmentFolders.map(async (workspaceFolder) => {
        ...
        for (const include of getCombinedConfig(this.config, workspaceFolder)
          .include) {
          ...
          const filter = (v: vscode.Uri) =>
            exclude.every(
              (x) =>
                !minimatch(path.relative(workspacePath, v.fsPath), x, {
                  dot: true,
                }),
            );
          watcher.onDidCreate(
  ...
  public discoverTestFromDoc(
    ...
    if (!shouldIncludeFile(e.uri.fsPath, this.config)) return;
  ...
  discoverTestFromPath(
    ...
    if (!data.resolved || forceReload) data.updateFromDisk(controller);
...
export function discoverTestFromFileContent(
  ...
  function getParent(block: NamedBlock): (typeof ancestors)[number] {
    ...
    if (parent.block == null) return parent;
