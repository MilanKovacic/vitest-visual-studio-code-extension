...
import type { ParsedNode, ParsedNodeType } from "./parser-nodes";
import { NamedBlock, ParseResult, ParsedRange } from "./parser-nodes";
...
export const parse = (
  ...
  const getNameForNode = (node: any) => {
    if (isFunctionCall(node) && node.expression.callee) {
      ...
      const property =
        rootCallee.property?.name || deepGet(rootCallee, "tag").property?.name;
      const name =
        rootCallee.name ||
  ...
  const isAnExpect = (node: any) => {
    if (!isFunctionCall(node)) return false;
  ...
  const searchNodes = (babylonParent: any, parent: ParsedNode) => {
    ...
    if (!babylonParent.body || !Array.isArray(babylonParent.body)) return;
