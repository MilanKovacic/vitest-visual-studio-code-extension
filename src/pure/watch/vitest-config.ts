...
async function connectAndFetchConfig({
  port,
  url = `ws://localhost:${port}/__vitest_api__`,
  reconnectInterval,
  reconnectTries,
}: {
  url?: string;
  ...
  return new Promise<ResolvedConfig>((resolve, reject) => {
    onFailedConnection = () =>
      reject(new Error("Unable to connect to Vitest API"));
...
export async function fetchVitestConfig(
  ...
  const workspace = workspaceConfigs.find(
    (workspace) =>
      workspace.isCompatible &&
      !workspace.isDisabled &&
      workspace.isUsingVitestForSure,
  );
  if (!workspace) return;
