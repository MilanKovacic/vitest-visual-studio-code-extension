...
export class TestRunner {
  ...
  async scheduleRun(
    ...
    if (updateSnapshot) args.push("--update");
    ...
    if (testNamePattern) {
      ...
      if (isWindows && !customStartProcess)
        args.push(
          "-t",
          `"${testNamePattern.replace(/[$^+?()[\]"]/g, "\\$&")}"`,
        );
      else args.push("-t", testNamePattern.replace(/[$^+?()[\]"]/g, "\\$&"));
    }
    ...
    const output = await runVitestWithApi(
      { cmd: sanitizeFilePath(command), args },
      this.workspacePath,
      {
        log: (line) => {
        ...
        onFinished: (files) => {
          if (files == null) {
            if (!cancellation?.isCancellationRequested) handleError();
            return;
          }
...
export async function getNodeVersion() {
  ...
  for await (const line of chunksToLinesAsync(process.stdout)) return line;
}
