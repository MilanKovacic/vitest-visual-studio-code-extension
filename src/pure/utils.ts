...
export function getVitestCommand(
  ...
  if (!projectRoot || projectRoot.length < 5) return;
  ...
  try {
    ...
    const suffixes = [""];
    if (isWindows) suffixes.unshift(".cmd", ".CMD");
 ...
 */
export const detectVitestVersion = async (
  command: string,
  args: string[],
  ...
  const versionExecPath = await spawnVitestVersion(
    process.execPath,
    [command, ...args],
    envs,
  );

  if (versionExecPath !== undefined) return versionExecPath;

  throw new Error(
    "Cannot get vitest version. Please open an issue at https://github.com/vitest-dev/vscode/issues and join the logs above.",
  );
};
...
export async function getVitestVersion(
  ...
  return await detectVitestVersion(
    vitestCommand.cmd,
    [...vitestCommand.args, "-v"],
    envs,
  );
}
...
export function sanitizeFilePath(path: string) {
  if (isWindows) return replaceDoubleSlashes(capitalizeDriveLetter(path));
...
export function execWithLog(
  ...
  const promise = Promise.allSettled([
    ...
    (async () => {
      for await (const line of chunksToLinesAsync(child.stderr)) {
        if (error) error(line);
      }
    })(),
  ]);
...
export function negate(
  func: Function,
): (...args: typeof func.arguments) => Boolean {
