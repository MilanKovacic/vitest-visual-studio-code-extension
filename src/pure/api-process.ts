...
type Handlers = Partial<WebSocketEvents> & {
  log?: (msg: string) => void;
...
export class ApiProcess {
  ...
  async start() {
    ...
    const debouncedLog = (line: string) => {
      logs.push(line);
      if (this.recordOutput) this.output.push(line);
  ...
  kill() {
    ...
    if (this.process && this.process.pid) kill(this.process.pid);
    this.handlers.onFinished?.();
  }

  private _start(
    debouncedLog: (line: string) => void,
    port: number,
    cwd: string,
  ) {
